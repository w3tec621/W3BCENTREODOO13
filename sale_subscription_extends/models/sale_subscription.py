...
class SaleSubscriptionTemplate(models.Model):
    ...
    payment_mode = fields.Selection(selection_add=[('to_validate', 'Crear y validar')])
SaleSubscriptionTemplate()
...
class SaleSubscription(models.Model):
        ...
        if subscriptions:
            ...
            for company_id in set(data['company_id'][0] for data in sub_data):
                ...
                for subscription in subs:
                    ...
                    # invoice only
                    elif subscription.template_id.payment_mode in ['draft_invoice', 'manual', 'validate_send', 'to_validate']:
                        try:
                            invoice_values = subscription.with_context(lang=subscription.partner_id.lang)._prepare_invoice()
                            new_invoice = self.env['account.move'].with_context(context_invoice).create(invoice_values)
                            new_invoice.message_post_with_view(
                            ...
                            if subscription.template_id.payment_mode == 'to_validate':
                                subscription.validate_invoice(invoice=new_invoice)
    ...
    def validate_invoice(self, invoice=False):
        self.ensure_one()
        if invoice:
            invoice.action_post()
        return True
